...
const CHECKLIST = [
  { icon: Inbox, label: 'Import contacts', sub: `${DEMO.contacts.toLocaleString('en-US')} phone numbers`, at: 0.08, last: false },
  ...
  { icon: RefreshCw, label: 'Remove duplicates', sub: `${DEMO.dups} removed`, at: 0.26, last: false },
  { icon: CheckCircle2, label: 'Ready to message', sub: 'All systems go', at: 0.32, last: true },
];

const STATS = [
  { icon: Users, label: 'Total contacts', to: DEMO.contacts, at: 0.42 },
  { icon: BadgeCheck, label: 'Verified', to: DEMO.verified, at: 0.48 },
  { icon: RefreshCw, label: 'Duplicates removed', to: DEMO.dups, at: 0.54 },
];

const RESULTS = DEMO.verify.list;

const STATUS = {
  ready: { icon: Wifi, cls: 'border-primary/30 bg-success/10 text-primary', label: 'WhatsApp Ready' },
  dup: { icon: X, cls: 'border-red-500/25 bg-red-500/10 text-error', label: 'Duplicate Removed' },
  invalid: { icon: AlertCircle, cls: 'border-amber-500/30 bg-amber-500/10 text-warning', label: 'Invalid Number' },
};
...
function Ring({ mv, size = 80 }) {
  const r = 24;
  const c = 2 * Math.PI * r;
  const pct = useTransform(mv, (v) => easeOut(clamp01((v - 0.18) / 0.34)) * VERIFY_PCT);
  ...
  return (
    <div className="relative shrink-0" style={{ width: w, height: w }}>
      <svg width={w} height={w} viewBox={`0 0 ${w} ${w}`}>
        <circle cx={w / 2} cy={w / 2} r={r} fill="none" stroke="var(--gs-line)" strokeWidth="6" />
        <motion.circle
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <motion.span className="text-[13px] font-extrabold text-p-title">{num}%</motion.span>
...
function CheckItem({ icon: Icon, label, sub, at, last, mv }) {
  ...
  return (
    <motion.div style={{ opacity: op, x }} className={cn('flex items-center gap-2 rounded-lg border px-2 py-1.5', last ? 'border-primary/30 bg-primary/[0.07]' : 'border-line/70 bg-p-chip')}>
      <span className={cn('flex h-[13px] w-[13px] shrink-0 items-center justify-center rounded-full border', last ? 'border-primary/60 bg-primary' : 'border-line bg-p-chip-strong')}>
        <motion.span style={{ opacity: tickOp, scale: tickSc }} className="text-[#07130f]">
          <Check size={8} strokeWidth={3.5} />
        </motion.span>
      </span>
      <Icon size={11} className={cn('shrink-0', last ? 'text-primary' : 'text-p-faint')} strokeWidth={2} />
      <div className="min-w-0 flex-1">
        <p className="truncate text-[8.5px] font-bold text-p-body">{label}</p>
        <p className="truncate text-[7px] font-medium text-p-mut">{sub}</p>
      </div>
    </motion.div>
  );
}
...
function ResultRow({ row }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-line/60 bg-p-chip px-2 py-[6px]">
      <Avatar img={row.img} initials={initials(row.name)} tint={row.tint} size={22} />
      <div className="min-w-0 flex-1">
        <p className="truncate text-[8.5px] font-bold text-p-body">{row.name}</p>
        <p className="truncate text-[7px] font-medium text-p-mut">{row.phone} · {row.loc}</p>
      </div>
      <StatusPill status={row.status} at={row.at} mv={row._mv} />
      <MoreVertical size={10} className="shrink-0 text-p-faint" />
    </div>
  );
}
  ...
  return (
    <div className={cn('flex flex-col gap-1 px-1', className)}>
      <div className="flex items-center justify-between px-1 pb-[2px]">
        <span className="text-[7.5px] font-bold tracking-wide text-p-mut">VERIFICATION RESULTS</span>
        <span className="flex items-center gap-1 text-[7.5px] font-semibold text-primary">
          <CountUp mv={mv} a={0.52} b={0.68} to={DEMO.verified} /> verified
        </span>
      </div>
      {RESULTS.map((row) => (
    ...
    <div className="relative flex h-full min-h-0 flex-col" data-testid="verify-panel">
      ...
      <motion.div style={{ opacity: bodyOp, y: bodyY }} className="flex min-h-0 flex-1">
        ...
        <motion.aside style={{ opacity: leftOp }} className="hidden w-[200px] shrink-0 flex-col gap-1.5 border-r border-line/70 px-2 py-2 md:flex">
          <p className="px-1 pb-[2px] text-[7.5px] font-bold tracking-wide text-p-mut">VERIFY</p>
          {CHECKLIST.map((item) => (
        ...
        <motion.div style={{ opacity: ringCardOp }} className="flex min-w-0 flex-1 flex-col gap-[6px] px-2 pt-2 sm:px-3">
          <div className="flex items-center gap-3 rounded-xl border border-line/70 bg-p-chip px-2.5 py-2 sm:gap-4 sm:py-2.5">
            <Ring mv={mv} size={78} />
            <div className="min-w-0 flex-1">
              <motion.p style={{ opacity: ringLabelOp }} className="text-[9.5px] font-bold text-p-title">
                Checking WhatsApp availability…
              </motion.p>
              <motion.p style={{ opacity: ringLabelOp }} className="mt-[2px] text-[7.5px] font-medium text-p-mut">
                Phone numbers are matched against active WhatsApp accounts. {VERIFY_PCT}% verified so far.
              </motion.p>
              <div className="mt-2 h-1 w-full overflow-hidden rounded-full bg-p-chip-strong">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {STATS.map(({ icon: Icon, label, to, at }) => (
              <motion.div key={label} style={{ opacity: inRange(mv, at, at + 0.05) }} className="rounded-lg border border-line/70 bg-p-chip px-2 py-[7px]">
                <div className="flex items-center gap-1 text-p-mut">
                  <Icon size={9} className="text-primary/70" />
                  <span className="truncate text-[6.5px] font-semibold uppercase tracking-wide">{label}</span>
                </div>
                <p className="mt-[3px] text-[11px] font-extrabold text-p-title">
      ...
      <FooterStrip
        mv={mv}
        left={<><BadgeCheck size={9} className="text-primary" />{DEMO.verified.toLocaleString('en-US')} verified · {DEMO.dups} duplicates removed</>}
